          ...
          {profileData.projects.map((proj) => (
            ...
            >
              <div className="flex justify-between items-start">
                ...
                <span className="text-[13px] text-zinc-550 font-mono">{proj.year}</span>
              </div>
              <p className="text-[14px] text-zinc-400 mt-1">{proj.description}</p>
              <div className="flex flex-wrap gap-1 mt-2">
                {proj.tech.slice(0, 3).map((t) => (
                  <span key={t} className="text-[12px] bg-[#0c0c0e] border border-dark-border text-zinc-500 px-1.5 py-0.5 font-mono">
                    {t}
                  </span>
                ))}
              </div>
              <button 
                ...
                className="text-[13px] text-brand-cyan hover:underline mt-2.5 flex items-center gap-1 cursor-pointer font-mono"
              >
                View Details &rarr;
              </button>
            </div>
        ...
        <div className="mt-3 grid grid-cols-2 gap-1.5">
          ...
          <div className="col-span-2 text-center mt-1">
            <button 
              ...
              className="text-[13px] text-brand-violet hover:underline cursor-pointer font-mono"
            >
      ...
      return (
        <div className="mt-3 p-3 border border-dark-border bg-black space-y-3">
          ...
          <button 
            ...
            className="w-full text-center py-2 bg-[#0c0c0e] hover:bg-zinc-950 text-[13px] text-brand-cyan hover:underline cursor-pointer border border-dark-border font-mono transition-all"
          >
        ...
        <div className="mt-3 space-y-2">
          ...
          <div className="flex gap-2 justify-center">
            <a 
              ...
              target="_blank" 
              className="flex-1 flex items-center justify-center gap-1.5 py-2 border border-dark-border bg-black hover:bg-zinc-950 text-[14px] text-zinc-400 hover:text-white transition-colors font-mono"
            >
        ...
        <div className="mt-3 space-y-3">
          ...
          <div className="p-3 border border-dark-border bg-zinc-950/80">
            ...
            <div className="flex items-center justify-between gap-2 mt-2 p-1.5 bg-black border border-dark-border">
              <span className="text-[14px] text-zinc-300 font-mono truncate">{profileData.personal.email}</span>
              <button 
                onClick={handleCopyEmail}
                className="text-zinc-550 hover:text-white p-1 shrink-0 transition-colors cursor-pointer"
              >
                {copiedEmail ? <LuCheck className="w-3.5 h-3.5 text-green-400" /> : <LuCopy className="w-3.5 h-3.5" />}
              </button>
            </div>
          ...
          <div>
            <p className="text-[13px] text-zinc-550 font-semibold mb-1.5 uppercase tracking-wide font-mono">Suggested prompts:</p>
            <div className="grid grid-cols-1 gap-1">
              {suggestedPrompts.map((chip, idx) => (
                <button
                  key={idx}
                  onClick={() => handleSend(chip.query)}
                  className="text-left text-[14px] text-zinc-400 hover:text-brand-cyan hover:border-brand-cyan/30 px-3 py-2 border border-dark-border bg-black transition-all cursor-pointer font-mono"
                >
                  {chip.label}
                </button>
              ))}
